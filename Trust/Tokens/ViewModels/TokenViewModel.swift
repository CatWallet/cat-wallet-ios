...
final class TokenViewModel {
    ...
    func cellViewModel(for indexPath: IndexPath) -> TransactionCellViewModel {
        // pass transactionStore so that UI can know if there is user notes to allocate space
        return TransactionCellViewModel(transaction: tokenTransactionSections[indexPath.section].items[indexPath.row], config: config, chainState: ChainState(server: server), transactionsStore: transactionsStore, currentAccount: currentAccount, server: token.coin.server)
    }
